...
export async function GET(request: NextRequest) {
  ...
  const upstream = await fetch(`${CORE_URL}/reports/time-entries.pdf?${params}`, { headers: { "X-Actor-ID": String(actorId) }, cache: "no-store" });
  return new NextResponse(await upstream.arrayBuffer(), { status: upstream.status, headers: {
    "Content-Type": upstream.headers.get("Content-Type") ?? "application/pdf",
    "Content-Disposition": upstream.headers.get("Content-Disposition") ?? "attachment; filename=acmeworks-time-entries.pdf",
  }});
}
